...
def _aggregate_reviews(create_spark_session,review_scores_accuracy, review_scores_cleanliness, review_scores_checkin, review_scores_communication, review_scores_location, review_scores_value) -> float:
    """
...
def medallion_raw_to_bronze_dlt_transformation(random_df_str_nm) -> pyspark.sql.dataframe.DataFrame:
    ...
    bronze_df = spark.table(random_df_str_nm)
...
def medallion_silver_to_gold_dlt_transformation() -> pyspark.sql.dataframe.DataFrame:
    ...
    # Verify that df contains 1 row after dropping duplicates
    assert gold_df.filter(A.AttributesAdded.aggregated_review_scores.name == 7)
