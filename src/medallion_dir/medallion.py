...
class Medallion(IM.IMedallion):
    ...
    '''def imedallion_raw_to_bronze_transformation_from_external_database(self, mnt_path, fraction, seed, test_mode) -> S.dataframe.DataFrame:

        """
        Applies merge into for data read from Azure Data Lake Storage. Merge into updates the row if the join condition already exists and inserts a new row if it does not.

        :param mnt_path: The path where data read from Azure Data Lake Storage can be accessed in Databricks, often the folder called mnt. Should refer to the first file in the order you want to read e.g. airbnb_1.
        :type mnt_path: str
        ...
        if not SF.mount_path_exists_fn(mnt_path):
            raise FileNotFoundError(f"Please check that you provided a valid path to the first file (e.g. airbnb_1) in the mounted folder located in DBFS")
